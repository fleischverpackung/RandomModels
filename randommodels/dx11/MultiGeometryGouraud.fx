...
float4 PS(VSOut In): SV_Target
{
	...
	float4 col = inputTexture.Sample(linearSampler,float3(In.uv.xy, particleIndex % GeomCount)) * cAmb;
	 #if defined(KNOW_COLOR)
       col = inputTexture.Sample(linearSampler,float3(In.uv.xy, particleIndex % GeomCount)) * ParticleBuffer[In.particleIndex].color;
    #endif
